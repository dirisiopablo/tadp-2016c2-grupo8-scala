...
class MovimientoSuite extends FunSuite {
  ...
  test("Convertirse en Mono") {
    new MovimientoTest {
      ...
      v match {
        case Saiyajin(_, true, 0, true) if v.energia === vegeta.energiaMax * 3 && v.energia === v.energiaMax => succeed
        case _ => fail
      }
    }
  }
  ...
  test("Convertirse en Super Saiyajin") {
    new MovimientoTest {
      ...
      g match {
        case Saiyajin(_, false, 1, false) if g.energiaMax === goku.energiaMax * 5 && g.energia === goku.energia => succeed
        case _ => fail
      }
    }
  }
